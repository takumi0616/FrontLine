...
import japanize_matplotlib  # 日本語対応（pip install japanize-matplotlib）
import pandas as pd
...
from collections import deque  # BFS用
from skimage.morphology import skeletonize  # スケルトン化
...
import math
import pandas as pd
from skan import Skeleton  # skan のインストールが必要です（mamba install skan）
...
def create_comparison_videos(image_folder="./v30_result/visualizations/",
                             output_folder="./v30_result/",
                             frame_rate=4,  # フレームレート（通常画質）
                             low_res_scale=4,  # 低画質版の縮小率
                             low_res_frame_rate=2):  # 低画質版のフレームレート
    """
    月ごと及び年間の比較動画を生成する関数。
    """
    # 出力フォルダが存在しない場合作成
...
"""
3段階で前線を推定するサンプルプログラム（改善版）

- SwinTransformerのアルゴリズム部分はすでに実装済み（SwinTransformerSysなど）

使用コマンド:
- nohup python3 main_v30.py > out_v30.log 2>&1 &
"""
...
def print_memory_usage(msg=""):
    ...
    mem_info = process.memory_info().rss / 1024 / 1024  # MB単位
...
# 結果やモデルを保存する際のディレクトリ
stage1_out_dir = './v30_result/stage1_nc'
stage2_out_dir = './v30_result/stage2_nc'
stage3_out_dir = './v30_result/stage3_nc'
model_s1_save_dir = './v30_result/stage1_model'
model_s2_save_dir = './v30_result/stage2_model'
output_visual_dir = './v30_result/visualizations'
...
def get_available_months(start_year, start_month, end_year, end_month):
    """
    start_year, start_month から end_year, end_month までの期間の
    YYYYMM 文字列をリストで返す
    """
    months = []
...
class FrontalDatasetStage1(Dataset):
    """
    Stage1用のDataset
    93chの気象データ (前後6h, 現在, +6hを連結 => 93ch程度) を入力、前線(6クラス)をターゲットにする
    オンデマンド方式に変更：必要な時だけデータをロードし、メモリ使用量を削減
    """
    def __init__(self, months, nc_gsm_dir, nc_0p5_dir, cache_size=50):
        self.months = months
        self.nc_gsm_dir = nc_gsm_dir
        self.nc_0p5_dir = nc_0p5_dir
        self.data_index = []  # データの位置情報のみを保持
        self.lat = None
        self.lon = None
        self.cache = {}  # 最近アクセスされたデータのキャッシュ
        self.cache_size = cache_size  # キャッシュに保持するデータ数
        self.file_cache = {}  # ファイルハンドルキャッシュ
        self.file_cache_size = 10  # キャッシュするファイル数
        self.prepare_index()

    def prepare_index(self):
        """
        データのインデックスだけを構築（実際のデータはロードしない）
        """
        ...
        for month in self.months:
            ...
            # データセットのメタデータのみを読む
            # チャンク設定を最適化: 'auto'の代わりに具体的なチャンク設定を使用
    ...
    def load_single_item(self, idx):
        """
        指定されたインデックスのデータを読み込む
        最適化：ファイルキャッシュを使用し、チャンク設定で効率的にロード
        """
        ...
        if gsm_file in self.file_cache:
            ...
            # 効率的なチャンク設定とLZ4圧縮でオープン
            # time次元はチャンクサイズ1で、空間次元は全体でグループ化
            ds_gsm   = xr.open_dataset(gsm_file)
            
            # ファイルキャッシュが上限に達していれば、最も古いものを削除
    ...
    def __getitem__(self, idx):
        ...
        # 6クラス: 0=背景, 1=warm,2=cold,3=stationary,4=occluded,5=warm_cold
...
def evaluate_stage1(model, dataloader, save_nc_dir=None):
    """
    Stage1の評価
    混同行列の図 (Confusion Matrix) は作成せず、数値のみ出力
    """
    ...
    # stage1_confusion_matrix.png は作成しない
    ...
    if save_nc_dir is not None:
        ...
        for i in range(all_probs.shape[0]):
            ...
            probs_np = np.transpose(probs_np, (1,2,0))  # (lat,lon,class=6)
...
def run_stage1():
    ...
    if checkpoint_files:
        ...
        # ---- DataParallel あり/なしでキーを自動変換して読み込む ----
        state_dict = checkpoint["model_state_dict"]
        # DataParallel で学習したモデルの場合は 'module.' 接頭辞を除去
...
# --------------------------------------------------
# Stage 2 Dataset/Model
# --------------------------------------------------
class FrontalRefinementDataset(Dataset):
    """
    Stage2用のDataset
    元前線データ(5ch)をデフォルメ(膨張/ギャップ/...)し => 入力,
    正しいラベルを出力(6クラス)
    または stage1 の出力を読み込んで推論のみ
    オンデマンド方式に変更：必要な時だけデータをロードし、メモリ使用量を削減
    """
    ...
    def load_single_item(self, idx):
        ...
        if item.get('is_augmented', False) and 'front_file' in item:
            ...
            # データ拡張 - シード値が範囲内に収まるように修正
            np.random.seed((hash(str(time_dt)) + aug_idx) & 0x7FFFFFFF)  # 再現性のためのシード設定（0 ~ 2^31-1 の範囲内）
...
def evaluate_stage2(model, dataloader, save_nc_dir=None):
    ...
    if save_nc_dir is not None:
        ...
        for i in range(all_probs.shape[0]):
            time_str=all_times[i]
            probs_np=all_probs[i].numpy()
            probs_np=np.transpose(probs_np,(1,2,0)) # (lat,lon,class)
            da=xr.DataArray(probs_np,dims=["lat","lon","class"],
                            coords={"lat":lat,"lon":lon,"class":np.arange(6)})
            ds=xr.Dataset({"probabilities":da})
            ds=ds.expand_dims('time')
...
def evaluate_stage3(stage2_nc_dir, save_nc_dir):
    """
    Stage2の結果 (6クラス) からクラス1～5をまとめてスケルトン化
    """
    ...
    for f in tqdm(stage2_files, desc="[Stage3] Evaluate"):
        ...
        # スケルトン化処理
        class_map = np.argmax(probs_np, axis=-1)  # (lat,lon)
        ...
        file_end = time.time()
        if (stage2_files.index(f) + 1) % 10 == 0:  # 10ファイルごとに時間を表示
...
def visualize_results(stage1_nc_dir,stage2_nc_dir,stage3_nc_dir,original_nc_dir,output_dir):
    """
    Stage1, Stage2, Stage3, および元データを比較表示
    """
    print("可視化処理を開始します。")
...
def compute_metrics(y_true, y_pred, labels):
    """
    全体精度、Macro 平均 Precision, Recall, F1、Cohen's kappa を計算する関数。
    y_true, y_pred は 1次元の配列、labels は評価対象クラスのリスト
    戻り値は (Accuracy, Macro Precision, Macro Recall, Macro F1, Cohen’s kappa)
    """
...
def run_evaluation():
    """
    2023年データに対して Stage1 / Stage2 / Stage3 の結果と
    nc_0p5_bulge_v2 (6クラス) の正解を比較し、
    (1) 各ステージの混同行列（6×6：全クラス）の図を表示、
    (2) 各ステージでの Overall Accuracy, Macro Precision, Macro Recall, Macro F1, Cohen's κ を算出し
        表形式で表示します。
    さらに、前線なし（背景，クラス0）を除いた前線部分（クラス1～5）の評価結果表も新たに作成し、
    ひとつの画像内にまとめます。
    図の横幅も広め（16×12）に設定し、混同行列等が見やすくなっています。
    最終的な画像は "v30_result/evaluation_summary.png" に保存されます。
    
    追加機能：
    (3) 時刻単位におけるピクセル単位の前線（クラス1〜5）の一致率を算出し、
        全体の平均値や中央値、最頻値などを計算。全体的に前線予測がどの程度一致しているかを
        数値で可視化します。
    """
    ...
    # 時刻単位の前線一致率分析のための出力ディレクトリ
    pixel_analysis_dir = "./v30_result/pixel_analysis"
    os.makedirs(pixel_analysis_dir, exist_ok=True)
    ...
    for key in common_keys:
        ...
        probs_s1 = ds1['probabilities'].isel(time=0).values  # (lat, lon, class=6)
        ds1.close()
    ...
    # 評価結果をまとめた Figure を作成
    # 横幅を広げ、全体サイズを 16×12 に設定
    fig = plt.figure(figsize=(16, 12))
    gs = fig.add_gridspec(3, 3, height_ratios=[3, 1, 1], hspace=0.4)
    
    # 混同行列のプロット（1行目：各ステージの全クラス対象）
    # 色のスケールを正規化して各クラスの数の違いを考慮する
    # 行ごとに正規化することで、各実際のクラスがどのように予測されたかを明確にする
    def normalize_cm(cm):
    ...
    sns.heatmap(cm_s1_norm, annot=cm_s1, fmt='d', cmap='Blues',
                ...
                vmin=0, vmax=1.0)  # 正規化後の値範囲を0～1に固定
    ...
    # 評価指標テーブル（前線のみ：クラス1～5）の表示（3行目全体）
    ...
    # 追加テキスト： 時刻ごとのクラス集合一致率
    summary_text = (f"Time-wise Presence Set match (%)\n"
                    f"  Stage1: {ratio_s1:.2f}%, Stage2: {ratio_s2:.2f}%, Stage3: {ratio_s3:.2f}%")
    fig.text(0.5, 0.02, summary_text, ha='center', fontsize=12)
    
    plt.tight_layout(rect=[0, 0.05, 1, 1])
    out_fig = "v30_result/evaluation_summary.png"
    plt.savefig(out_fig, dpi=300)
    plt.close()
    print(f"[Evaluation] Done. Figure -> {out_fig}")
    
    # 3. 時刻単位におけるピクセル単位の前線（クラス1〜5）の一致率分析
    print("[Evaluation] Starting pixel-level front analysis...")
    
    # 時刻ごとの分析結果を格納するリスト
    pixel_analysis_results = []
    
    # 各前線クラスのピクセル数と一致ピクセル数を格納する辞書
    class_names = {1: "warm", 2: "cold", 3: "stationary", 4: "occluded", 5: "warm_cold"}
    
    for key_idx, key in enumerate(common_keys):
        # 進捗表示
        if (key_idx + 1) % 10 == 0 or key_idx == 0 or key_idx == len(common_keys) - 1:
            print(f"[Evaluation] Processing timestamp {key_idx+1}/{len(common_keys)}: {key}")
        
        # 正解データ
        gt_map = gt_list[key_idx]
        
        # 各ステージの予測データ
        pred_s1 = stage1_pred_list[key_idx]
        pred_s2 = stage2_pred_list[key_idx]
        pred_s3 = stage3_pred_list[key_idx]
        ...
        for cls in range(1, 6):
            # 正解データの前線ピクセル数
            gt_pixels = np.sum(gt_map == cls)
            result[f"gt_class{cls}_pixels"] = int(gt_pixels)
            
            # Stage1の予測データのピクセル数と一致ピクセル数
            s1_pixels = np.sum(pred_s1 == cls)
            s1_match_pixels = np.sum((pred_s1 == cls) & (gt_map == cls))
            result[f"stage1_class{cls}_pixels"] = int(s1_pixels)
            result[f"stage1_class{cls}_match_pixels"] = int(s1_match_pixels)
            if gt_pixels > 0:
                result[f"stage1_class{cls}_match_rate"] = float(s1_match_pixels) / gt_pixels
            else:
                result[f"stage1_class{cls}_match_rate"] = float('nan')
            
            # Stage2の予測データのピクセル数と一致ピクセル数
            s2_pixels = np.sum(pred_s2 == cls)
            s2_match_pixels = np.sum((pred_s2 == cls) & (gt_map == cls))
            result[f"stage2_class{cls}_pixels"] = int(s2_pixels)
            result[f"stage2_class{cls}_match_pixels"] = int(s2_match_pixels)
            if gt_pixels > 0:
                result[f"stage2_class{cls}_match_rate"] = float(s2_match_pixels) / gt_pixels
            else:
                result[f"stage2_class{cls}_match_rate"] = float('nan')
            
            # Stage3の予測データのピクセル数と一致ピクセル数
            s3_pixels = np.sum(pred_s3 == cls)
            s3_match_pixels = np.sum((pred_s3 == cls) & (gt_map == cls))
            result[f"stage3_class{cls}_pixels"] = int(s3_pixels)
            result[f"stage3_class{cls}_match_pixels"] = int(s3_match_pixels)
            if gt_pixels > 0:
                result[f"stage3_class{cls}_match_rate"] = float(s3_match_pixels) / gt_pixels
            else:
                result[f"stage3_class{cls}_match_rate"] = float('nan')
        
        # 全クラス(1-5)の前線ピクセル数と一致ピクセル数の総計
        all_front_gt = np.sum((gt_map >= 1) & (gt_map <= 5))
        result["gt_all_front_pixels"] = int(all_front_gt)
        
        # Stage1の全前線ピクセル一致率
        all_front_s1 = np.sum((pred_s1 >= 1) & (pred_s1 <= 5))
        all_front_s1_match = np.sum(((pred_s1 >= 1) & (pred_s1 <= 5)) & ((gt_map >= 1) & (gt_map <= 5)) & (pred_s1 == gt_map))
        result["stage1_all_front_pixels"] = int(all_front_s1)
        result["stage1_all_front_match_pixels"] = int(all_front_s1_match)
        if all_front_gt > 0:
            result["stage1_all_front_match_rate"] = float(all_front_s1_match) / all_front_gt
        else:
            result["stage1_all_front_match_rate"] = float('nan')
        
        # Stage2の全前線ピクセル一致率
        all_front_s2 = np.sum((pred_s2 >= 1) & (pred_s2 <= 5))
        all_front_s2_match = np.sum(((pred_s2 >= 1) & (pred_s2 <= 5)) & ((gt_map >= 1) & (gt_map <= 5)) & (pred_s2 == gt_map))
        result["stage2_all_front_pixels"] = int(all_front_s2)
        result["stage2_all_front_match_pixels"] = int(all_front_s2_match)
        if all_front_gt > 0:
            result["stage2_all_front_match_rate"] = float(all_front_s2_match) / all_front_gt
        else:
            result["stage2_all_front_match_rate"] = float('nan')
        
        # Stage3の全前線ピクセル一致率
        all_front_s3 = np.sum((pred_s3 >= 1) & (pred_s3 <= 5))
        all_front_s3_match = np.sum(((pred_s3 >= 1) & (pred_s3 <= 5)) & ((gt_map >= 1) & (gt_map <= 5)) & (pred_s3 == gt_map))
        result["stage3_all_front_pixels"] = int(all_front_s3)
        result["stage3_all_front_match_pixels"] = int(all_front_s3_match)
        if all_front_gt > 0:
            result["stage3_all_front_match_rate"] = float(all_front_s3_match) / all_front_gt
        else:
            result["stage3_all_front_match_rate"] = float('nan')
        
        pixel_analysis_results.append(result)
    
    # 時刻ごとの詳細データをCSVに保存
    detailed_df = pd.DataFrame(pixel_analysis_results)
    detailed_csv_path = os.path.join(pixel_analysis_dir, "pixel_analysis_detailed.csv")
    detailed_df.to_csv(detailed_csv_path, index=False)
    print(f"[Evaluation] Detailed pixel analysis saved to {detailed_csv_path}")
    ...
    for col in detailed_df.columns:
        ...
        # 数値データのみ処理
        if pd.api.types.is_numeric_dtype(detailed_df[col]):
            # NaNを除外してから計算
            valid_data = detailed_df[col].dropna()
            if len(valid_data) > 0:
                stats_data[f"{col}_sum"] = valid_data.sum()
                stats_data[f"{col}_mean"] = valid_data.mean()
                stats_data[f"{col}_median"] = valid_data.median()
                # 最頻値（複数ある場合は最初のもの）
                mode_result = valid_data.mode()
                if len(mode_result) > 0:
                    stats_data[f"{col}_mode"] = mode_result[0]
                else:
                    stats_data[f"{col}_mode"] = float('nan')
    
    # 各クラスと各ステージの平均一致率をまとめた表も作成
    match_rate_summary = {}
    
    for cls in range(1, 6):
        for stage in ["stage1", "stage2", "stage3"]:
            col = f"{stage}_class{cls}_match_rate"
            match_rate_summary[f"{stage}_class{cls}"] = detailed_df[col].mean()
    
    # 全クラス合計の平均一致率
    for stage in ["stage1", "stage2", "stage3"]:
        col = f"{stage}_all_front_match_rate"
        match_rate_summary[f"{stage}_all_front"] = detailed_df[col].mean()
    
    # 統計情報をCSVに保存
    stats_df = pd.DataFrame([stats_data])
    stats_csv_path = os.path.join(pixel_analysis_dir, "pixel_analysis_statistics.csv")
    stats_df.to_csv(stats_csv_path, index=False)
    print(f"[Evaluation] Statistical summary saved to {stats_csv_path}")
    
    # 一致率のサマリーをCSVに保存
    match_rate_df = pd.DataFrame([match_rate_summary])
    match_rate_csv_path = os.path.join(pixel_analysis_dir, "pixel_analysis_match_rate_summary.csv")
    match_rate_df.to_csv(match_rate_csv_path, index=False)
    print(f"[Evaluation] Match rate summary saved to {match_rate_csv_path}")
    ...
    for cls in range(1, 6):
        plt.subplot(2, 3, cls)
        for stage, color in zip(["stage1", "stage2", "stage3"], ["blue", "green", "red"]):
            col = f"{stage}_class{cls}_match_rate"
            plt.plot(detailed_df["timestamp"], detailed_df[col], color=color, label=stage)
    ...
    plt.title("All Front Classes (1-5)")
    plt.xlabel("Time")
    plt.ylabel("Match Rate")
    plt.ylim(0, 1.0)
    plt.xticks(rotation=45)
    ...
    plt.tight_layout()
    match_rate_plot_path = os.path.join(pixel_analysis_dir, "pixel_analysis_match_rate_plot.png")
    plt.savefig(match_rate_plot_path, dpi=300)
    plt.close()
    print(f"[Evaluation] Match rate plot saved to {match_rate_plot_path}")
    
    print("[Evaluation] Pixel-level front analysis completed.")


# --------------------------------------------------
# Stage 4: スケルトンから中心線(点列)を抽出しSVGで保存
# --------------------------------------------------
...
def smooth_polyline(points, window_size=3):
    """
    与えられた点列（リスト of (x, y)）に対して移動平均による平滑化を行う。
    点数がwindow_size以下の場合はそのまま返す。
    """
        ...
        try:
            ...
            if "index pointer size" in str(e):
                ...
                if len(ys) > 0:
                    ...
                    # 同じ点を2回用いて、ポリラインとして出力（点印として描画される）
...
def save_polylines_as_svg(polylines, viewBox, output_path, smoothing_window=3):
    """
    抽出したポリライン群をSVGファイルとして保存する。
    viewBox は (min_lon, min_lat, width, height) のタプル。
    グリッド線と軸ラベルを追加して出力する。
    """
    # 前線クラスごとの色設定
    class_colors = {
...
##############################################################################
# ＜Stage4：Skeleton化結果からSVG出力＞
##############################################################################
def evaluate_stage4(stage3_nc_dir, output_svg_dir):
    """
    指定されたStage3のncファイルからclass_mapを読み出し、
    extract_polylines_using_skan()で各前線クラスごとのポリラインを抽出し、
    SVGファイルとしてoutput_svg_dirに保存する。
    """
    ...
    for f in skeleton_files:
        ...
        # viewBoxはlon[::-1]、lat[::-1]の最小・最大値から決定
        lon_fixed = lon[::-1]
        lat_fixed = lat[::-1]
...
def run_stage4():
    """
    改善版Stage4を実行する関数。
    指定したStage3のskeleton_ncファイルディレクトリから読み込み、evaluate_stage4()を実行し、
    SVGファイルを指定の出力ディレクトリに保存する。
    """
    stage3_nc_dir = "./v30_result/stage3_nc"
    output_svg_dir = "./v30_result/stage4_svg"
    evaluate_stage4(stage3_nc_dir, output_svg_dir)
    print("【Stage4 Improved】 SVG 出力処理が完了しました。")

# ======================================================================
#  Stage-1  SHAP 解析  （CPU4スレッド / GPU 自動切替）
# ======================================================================
# ───────── BLAS スレッド固定 ─────────
...
def _load_model(ckpt:str, device):
    from main_v30 import SwinUnetModel          # 既存クラスを再利用
...
class OnlineMoments:
    """
    SHAP 値 (チャネル, H, W)=(93,128,128) を想定した逐次統計.
    channels-last (H,W,C) で来ても自動で transpose して処理する。
    """
    ...
    def _to_CHW(self, arr:np.ndarray)->np.ndarray:
        """
        形状を (C,H,W) = (93,128,128) に変換
        ・arr.ndim == 4 の場合 : 先頭がバッチ or 出力次元 -> squeeze(0)
        ・channels‐last の場合  (H,W,93) -> transpose
        """
        arr = np.squeeze(arr)          # (B,C,H,W) → (C,H,W)  or  (H,W,C)
        if arr.ndim != 3:
    ...
    def update(self, arr):
        """
        arr : ndarray / tensor  (93,128,128) もしくは (1,128,93) など
        """
        if isinstance(arr, torch.Tensor):
            arr = arr.cpu().numpy()
        arr = self._to_CHW(arr)

        abs_m = np.abs(arr).mean(axis=(1,2))   # (93,)
        sig_m = arr.mean(axis=(1,2))           # (93,)
...
def run_stage1_shap_evaluation_cpu(use_gpu=True,
                                   ...
                                   out_root="./v30_result/shap_stage1"):
    print("\n========== Stage-1 SHAP 解析 ==========")
    # ---- データセット（2023年のみ）----
    ...
    for idx in tqdm(idxs,desc="Compute SHAP"):
        ...
        for c in present:
            ...
            xx=x.unsqueeze(0).to(device)                 # (1,93,128,128)
            sv=_safe_shap(expl[c],xx,ns=16)              # list or ndarray
            val=sv[0] if isinstance(sv,list) else sv     # (1,93,128,128) or (1,128,128,93)
            if isinstance(val, torch.Tensor):
                val = val.cpu().numpy()
            # ---- 形状を (93,128,128) に整形 ----
            val = np.squeeze(val)                        # remove batch/output dim → (93,128,128) or (128,128,93)
            if val.shape[-1]==93 and val.shape[0]!=93:    # channels-last → transpose
                val = np.transpose(val, (2,0,1))
            stats[c].update(val)                         # ← 修正後の update が自動判断
            Sbuf[c].append(val.mean((1,2)))              # (93,)
...
def main():
    total_start_time = time.time()  # プログラム全体の開始時間
